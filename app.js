...
const APP_CHANGELOG=[
  {v:"2.1.0",date:"01/06/2025",items:["Reset besoins PVP à 23h automatique","Fix erreur sauvegarde Salad Bar","DLC du jour en rouge en tête de liste","Badge nombre de DLC sur DLC Frais","Étiquette DLC dans DLC Frais","Too Good To Go regroupé dans DLC Frais","Écran connexion : version + date MAJ + changelog","Tri amélioré par famille + date DLC","Planning style Connecteam refonte complète","Ajout en rayon après coché PVP avec choix DLC 24h/48h/72h"]},
...
window.app={
    ...
    toggleChangelog:function(){
      ...
      if(!visible){
        ...
        APP_CHANGELOG.forEach(entry=>{
          html+=`<div style="margin-bottom:10px;"><div style="font-weight:800;color:#1d4ed8;font-size:12px;">v${entry.v} — ${entry.date}</div><ul style="margin:4px 0 0 14px;padding:0;">`;
          entry.items.forEach(it=>{html+=`<li style="font-size:11px;color:#374151;margin-bottom:2px;">${it}</li>`;});
          html+='</ul></div>';
        });
        el.innerHTML=html;
      }
      el.style.display=visible?'none':'block';
      if(icon)icon.className=visible?'fas fa-gear':'fas fa-times';
    }
  },
  ...
  save:async function(sec){
    ...
    if(sec==='salad'){const cutoff=Date.now()-(24*60*60*1000);const cleanStock=(this.data.saladStock||[]).filter(x=>!x.end||(x.end>cutoff));await setDoc(docSalad,{lib:this.data.saladLib||[],stock:cleanStock});this.data.saladStock=cleanStock;}
