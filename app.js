...
const docSaladStock=doc(db,"store_master","DATA_SALAD_STOCK");
...
// Modules présents dans l'application
const APP_MODULES=[
  {icon:"fa-calendar-xmark",label:"DLC Frais",desc:"Scan code-barres · Liste triée par famille · Tri urgences · Étiquettes DLC"},
  {icon:"fa-store",label:"Besoins PVP",desc:"Checklist besoins rayon · Sortie en rayon avec DLC 24/48/72h · Reset 23h"},
  {icon:"fa-bowl-food",label:"Salad Bar",desc:"Bibliothèque produits · Gestion stock en rayon · Traçabilités photo"},
  {icon:"fa-snowflake",label:"Températures Frigo",desc:"Relevés multi-frigos · Historique 90 jours · Alertes restants"},
  {icon:"fa-list-check",label:"Check-list",desc:"Tâches par zone · Validation avec confirmation · Notes textuelles"},
  {icon:"fa-truck",label:"Réception",desc:"Gestion fournisseurs · Planning livraisons · Traçabilité réception"},
  {icon:"fa-clock",label:"Pointeuse",desc:"Pointage arrivée/départ · Gestion pause · Timesheet semaine · Dépointage"},
  {icon:"fa-calendar-days",label:"Planning RH",desc:"Grille semaine · Drag & drop · Profils RH légaux · Résumé heures"},
  {icon:"fa-comments",label:"Chat interne",desc:"Canaux par sujet · Messages temps réel · Gestion accès par canal"},
  {icon:"fa-clipboard-check",label:"Inspection",desc:"Contrôles hygiène · Export CSV · Photos conformité"},
  {icon:"fa-tag",label:"Étiquettes DLC",desc:"Génération étiquettes · Impression · Gestion dates"},
  {icon:"fa-store-alt",label:"Gestion magasin",desc:"Planning réception · Tableau de bord · Accès rapides"},
];
...
window.app={
  ...
  auth:{
    ...
    initLoginScreen:function(){
      ...
      const dEl=document.getElementById('login-update-date');
      if(vEl)vEl.textContent=APP_VERSION;
      if(dEl)dEl.textContent=APP_LAST_UPDATE;
    },
    toggleChangelog:function(){
      const el=document.getElementById('login-changelog');
      const icon=document.getElementById('changelog-icon');
      if(!el)return;
      const visible=el.style.display!=='none';
      if(!visible){
        let html='<div style="font-weight:900;color:#1e40af;font-size:12px;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #bfdbfe;">📦 Modules disponibles</div>';
        APP_MODULES.forEach(m=>{
          html+=`<div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:8px;">
            <i class="fas ${m.icon}" style="color:#0891b2;width:14px;margin-top:1px;flex-shrink:0;"></i>
            <div><div style="font-weight:800;color:#1e293b;font-size:12px;">${m.label}</div>
            <div style="font-size:10px;color:#64748b;">${m.desc}</div></div>
          </div>`;
        });
        el.innerHTML=html;
      }
      el.style.display=visible?'none':'block';
      if(icon)icon.className=visible?'fas fa-th-list':'fas fa-times';
    }
  },
  ...
  loadAll:async function(){
    ...
    if(sS.exists()){const d=sS.data();this.data.saladLib=d.lib||[];}else{this.data.saladLib=[];}
    // Load stock from separate doc (avoids 1MB limit)
    try{const ssSnap=await getDoc(docSaladStock);if(ssSnap.exists()){const d=ssSnap.data();const cutoff=Date.now()-(24*60*60*1000);this.data.saladStock=(d.stock||[]).filter(x=>!x.end||(x.end>cutoff));}else{this.data.saladStock=[];}}catch(e){this.data.saladStock=[];}
  ...
  _startPvpMidnightReset(){
    ...
    this._pvpResetTimer=setTimeout(async ()=>{
      if(this.data.checkedNeeds?.length){
        ...
        toast('✅ Besoins PVP réinitialisés pour demain (23h)','info',4000);
      }
  ...
  save:async function(sec){
    ...
    if(sec==='salad'){
      ...
      const cleanStock=(this.data.saladStock||[]).filter(x=>!x.end||(x.end>cutoff));
      this.data.saladStock=cleanStock;
      // Save lib and stock in SEPARATE documents to avoid 1MB Firestore limit
      await Promise.all([
        setDoc(docSalad,{lib:this.data.saladLib||[],updatedAt:Date.now()}),
        setDoc(docSaladStock,{stock:cleanStock,updatedAt:Date.now()})
      ]);
    }
  ...
  updateDlcBadges(){
    ...
    const fc=document.getElementById('badge-frais-count');
    if(fc){if(fraisCount>0){fc.style.display='flex';fc.textContent=fraisCount;}else fc.style.display='none';}
  ...
  PERMS:{
    // ── DLC ──
    saladLib:    {label:'Modifier bibliothèque Salad Bar', icon:'fa-leaf',           group:'SALAD BAR'},
    saladStock:  {label:'Saisir stock Salad Bar',          icon:'fa-bowl-food',      group:'SALAD BAR'},
    pvpLib:      {label:'Modifier bibliothèque PVP',       icon:'fa-store',          group:'PVP / RAYON'},
    pvpStock:    {label:'Saisir stock PVP',                icon:'fa-boxes-stacked',  group:'PVP / RAYON'},
    fraisEdit:   {label:'Modifier / supprimer Frais',      icon:'fa-calendar-xmark', group:'DLC FRAIS'},
    // ── TEMPÉRATURES ──
    ...
    // ── RÉCEPTION ──
    receptionEdit:{label:'Saisir / modifier réceptions',   icon:'fa-truck',          group:'RÉCEPTION'},
    // ── PLANNING ──
    planning:    {label:'Accès Planning RH',               icon:'fa-calendar-days',  group:'PLANNING RH'},
    // ── POINTEUSE ──
    pointeuse:   {label:'Utiliser la pointeuse',           icon:'fa-clock',          group:'POINTEUSE'},
    pointeuseAdmin:{label:'Dépointage & admin pointeuse',  icon:'fa-user-clock',     group:'POINTEUSE'},
    // ── OUTILS CARREFOUR ──
    dynacad:     {label:'Accès Dynacad',                   icon:'fa-store',          group:'OUTILS CARREFOUR'},
    meti:        {label:'Accès METI (VPN Carrefour)',      icon:'fa-laptop',         group:'OUTILS CARREFOUR'},
  },
  ...
  frais:{
    ...
    tab(t){
      ...
      const tabMap={scan:'tf-1',list:'tf-2'};
      document.getElementById(tabMap[t]).classList.add('active');
      ['frais-scan','frais-list'].forEach(id=>{const el=document.getElementById(id);if(el)el.classList.add('hidden');});
      document.getElementById('frais-'+t).classList.remove('hidden');
      if(t==='list'){this.render();}
      // tgtg moved to DLC home card
    },
    toggleScan(){
    ...
    tgtgDeduct:async function(id,fromScan=false,motif='tgtg'){
      ...
      const motifLabel=motif==='promo50'?'Étiquette DLC':'Too Good To Go';
      if(currentQty<=1){
        // Remove completely
    ...
    render(){
      ...
      // Nomenclature officielle Carrefour (ordre rayon)
      const NOMEN_ORDER={"EPICERIE":1,"BISCOTTES":2,"PAIN DE MIE":3,"BISCUITERIE":4,"VIENNOISERIE":5,"PRODUITS APERITIF":6,"CAFE":7,"PETITS DEJEUNERS":8,"CEREALES":9,"THE INFUSION":10,"CHOCOLAT":11,"CONFISERIE":12,"DESSERTS":13,"LAITS":14,"SUCRES":15,"FARINES":16,"PATES":17,"RIZ":18,"CONSERVES":19,"PLATS CUISINES":20,"LIQUIDES":50,"BIERES":51,"EAUX":52,"VINS":53,"ALCOOLS":59,"DPH":100,"HYGIENE":110,"BEAUTE":120,"PARAPHARMACIE":130,"BAZAR":150,"FRUITS & LEGUMES":300,"FRUITS ET LEGUMES":300,"F&L":300,"BOUCHERIE":310,"VOLAILLE":311,"HMR":320,"ROTISSERIE":320,"CHARCUTERIE":330,"FROMAGE":340,"POISSON":350,"POINT CHAUD":360,"BOULANGERIE":361,"VIENNOISERIE FRAICHE":362,"DEPOT DE PAIN":400,"APLS":410,"TRAITEUR":410,"CHARCUTERIE LS":420,"BOUCHERIE LS":450,"VOLAILLE LS":460,"ULTRA FRAIS":470,"JUS":471,"BOF":480,"OEUFS":480,"FROMAGE LS":481,"LAIT":482,"CORPS GRAS":483,"SURGELES":500,"GLACES":501,"DIVERS":900};
      const famOrder=(fam)=>{const k=(fam||'').toUpperCase();for(const [key,v] of Object.entries(NOMEN_ORDER)){if(k.includes(key)||key.includes(k))return v;}return 800;};
      const sorted=filtered.slice().sort((a,b)=>{
        const da=new Date(a.date);const db=new Date(b.date);
        const aToday=!isNaN(da.getTime())&&da.getTime()===todayMidnight.getTime();
        const bToday=!isNaN(db.getTime())&&db.getTime()===todayMidnight.getTime();
        if(aToday&&!bToday)return -1;if(!aToday&&bToday)return 1;
        const aExp=!isNaN(da.getTime())&&da<todayMidnight;
        const bExp=!isNaN(db.getTime())&&db<todayMidnight;
        if(aExp&&!bExp)return -1;if(!aExp&&bExp)return 1;
        // Tri par ordre nomenclature Carrefour
        const famDiff=famOrder(a.fam)-famOrder(b.fam);
        if(famDiff!==0)return famDiff;
        return(a.fam||'').localeCompare(b.fam||'')||(da-db);
      });
      const todayCount=filtered.filter(i=>{const d=new Date(i.date);return!isNaN(d.getTime())&&d.getTime()===todayMidnight.getTime();}).length;
      const expiredCount=filtered.filter(i=>{const d=new Date(i.date);return!isNaN(d.getTime())&&d<todayMidnight;}).length;
      const badgeEl=document.getElementById('badge-frais-dlc');
      if(badgeEl){const tot=todayCount+expiredCount;badgeEl.style.display=tot>0?'inline-block':'none';badgeEl.textContent=tot+' à DLC';}
      const fc=document.getElementById('badge-frais-count');
      if(fc){const tot=todayCount+expiredCount;if(tot>0){fc.style.display='flex';fc.textContent=tot;}else fc.style.display='none';}
      const grp={};
      sorted.forEach(i=>{if(!grp[i.fam])grp[i.fam]=[];grp[i.fam].push(i);});
      ...
      if(todayCount>0||expiredCount>0){
        parts.push(`<div style="background:#fee2e2;border-left:5px solid #ef4444;border-radius:12px;padding:10px 14px;margin-bottom:10px;display:flex;align-items:center;gap:10px;"><i class="fas fa-triangle-exclamation" style="color:#dc2626;font-size:18px;"></i><div><div style="font-weight:950;color:#991b1b;font-size:14px;">${expiredCount>0?expiredCount+' DLC dépassé · ':''}${todayCount>0?todayCount+' DLC aujourd\'hui':''}</div><div style="font-size:11px;color:#b91c1c;">Ces produits apparaissent en tête de liste</div></div></div>`);
      }
      for(const f in grp){
        ...
        grp[f].forEach(i=>{
          ...
          const isExpired=!isNaN(d.getTime())&&d<todayMidnight;
          const isASaisir=window.app.common.isASaisir(i.date);
          ...
          const sc=isASaisir?`<span class="chip warn dlc-urgent-chip"><i class="fas fa-triangle-exclamation"></i> À SAISIR</span>`:'';
          const urgentStyle=(isToday||isExpired)?'border-left:4px solid #ef4444;background:linear-gradient(135deg,#fff5f5,#fff);':isASaisir?'border-left:4px solid #f97316;background:linear-gradient(135deg,#fff7ed,#fff);':'';
          const titleStyle=(isToday||isExpired)?'color:#dc2626;font-weight:950;':isASaisir?'color:#c2410c;font-weight:900;':'';
          const todayBadgeHtml=isToday?' <span style="font-size:10px;background:#ef4444;color:#fff;border-radius:8px;padding:2px 5px;">AUJOURD\'HUI</span>':isExpired?' <span style="font-size:10px;background:#7f1d1d;color:#fff;border-radius:8px;padding:2px 5px;">DÉPASSÉ</span>':'';
          parts.push(`<div class="swipe-wrapper"><div class="swipe-content" onclick="app.frais.openEdit(${i.id})" style="${urgentStyle}">
            <div style="display:flex;align-items:center;flex:1;min-width:0;gap:12px;">${img}<div class="item-info"><div class="item-title" style="${titleStyle}">${i.name||''}${todayBadgeHtml}</div><div class="item-sub">x${i.qty||0} • ${i.ean||'EAN ?'} ${sc}</div></div></div>
  ...
  pvp:{
    ...
    toggleNeed:async function(libId){
      const wasChecked=window.app.data.checkedNeeds.includes(libId);
      ...
      if(!wasChecked){
        const l=window.app.data.pvpLib.find(x=>x.id==libId);if(!l)return;
        const add=await dlgConfirm('Ajouter en rayon ?',`${l.name} — Ajouter directement en rayon et renseigner la DLC ?`);
        if(!add)return;
        const hours=await new Promise(resolve=>{
          ...
          modal.innerHTML=`<div style="background:#fff;border-radius:18px;padding:24px;max-width:320px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,.18);">
            <div style="font-weight:950;font-size:16px;margin-bottom:16px;"><i class="fas fa-store" style="color:#003896;margin-right:8px;"></i>DLC en rayon — <b>${l.name}</b></div>
            <div style="display:flex;gap:8px;margin-bottom:16px;" id="_dlc-btns">
              <button onclick="window._dlcH=24;document.querySelectorAll('#_dlc-btns button').forEach(b=>{b.style.background='#e2e8f0';b.style.color='#374151';});this.style.background='#003896';this.style.color='#fff';" style="flex:1;padding:14px;border:none;border-radius:10px;background:#e2e8f0;font-weight:900;font-size:16px;cursor:pointer;">24h</button>
              <button onclick="window._dlcH=48;document.querySelectorAll('#_dlc-btns button').forEach(b=>{b.style.background='#e2e8f0';b.style.color='#374151';});this.style.background='#003896';this.style.color='#fff';" style="flex:1;padding:14px;border:none;border-radius:10px;background:#e2e8f0;font-weight:900;font-size:16px;cursor:pointer;">48h</button>
              <button onclick="window._dlcH=72;document.querySelectorAll('#_dlc-btns button').forEach(b=>{b.style.background='#e2e8f0';b.style.color='#374151';});this.style.background='#003896';this.style.color='#fff';" style="flex:1;padding:14px;border:none;border-radius:10px;background:#e2e8f0;font-weight:900;font-size:16px;cursor:pointer;">72h</button>
            </div>
            <div style="display:flex;gap:8px;">
              <button onclick="modal.remove();resolve(null);" style="flex:1;padding:12px;border:none;border-radius:10px;background:#f1f5f9;font-weight:700;cursor:pointer;">Annuler</button>
              <button onclick="const h=window._dlcH;modal.remove();resolve(h||null);" style="flex:1;padding:12px;border:none;border-radius:10px;background:#003896;color:#fff;font-weight:900;cursor:pointer;">Confirmer</button>
            </div>
          </div>`;
          window._dlcH=null;
        ...
        if(!hours)return;
        const qty=await dlgPrompt('Quantité à mettre en rayon ?','1','number');
        if(!qty||Number(qty)<=0)return;
        const now=Date.now();
        const entry={id:now,libId,name:l.name,ean:l.ean,qty:Number(qty),out:now,end:now+(hours*60*60*1000),days:l.days||hours/24};
        window.app.data.pvpStock.push(entry);
        this.render();spin(true);
        try{await window.app.save('pvp');toast(`✅ ${qty}× ${l.name} en rayon — DLC dans ${hours}h`,'ok',4000);}
        catch(e){toast('Erreur sauvegarde.','err');}
        finally{spin(false);}
      }
    },
...
^XZ`;
  ...
  planning:{
    ...
    // ── MODÈLES RH LÉGAUX ──
    HR_LEGAL:{
      'Apprenti Mineur':{label:'Apprenti Mineur',icon:'👶',color:'#7c3aed',maxHoursDay:8,maxHoursWeek:35,minRestHours:12,minRestWeekHours:48,breakAfterH:4.5,breakMinutes:30,maxRestDaysWeek:2,nightBan:{start:'22:00',end:'06:00'},desc:'Code trav. L3163 — Max 8h/j · 35h/sem · Repos 12h · Interdit 22h–6h'},
      'Apprenti Majeur':{label:'Apprenti Majeur',icon:'🎓',color:'#0891b2',maxHoursDay:10,maxHoursWeek:48,minRestHours:11,minRestWeekHours:35,breakAfterH:6,breakMinutes:20,maxRestDaysWeek:2,desc:'Code trav. L3121-16 — Max 10h/j · 48h/sem · Pause 20min après 6h'},
      'CDI Temps Partiel':{label:'CDI Temps Partiel',icon:'🕐',color:'#d97706',maxHoursDay:10,maxHoursWeek:44,minRestHours:11,minRestWeekHours:35,breakAfterH:6,breakMinutes:20,minContractH:24,maxRestDaysWeek:2,desc:'Code trav. L3123 — Min 24h/sem · Heures compl. +10% (>10%) +25% (>1/10)'},
      'CDI Temps Complet':{label:'CDI Temps Complet',icon:'⭐',color:'#059669',maxHoursDay:10,maxHoursWeek:48,minRestHours:11,minRestWeekHours:35,breakAfterH:6,breakMinutes:20,refH:35,overtimeThreshold:35,maxRestDaysWeek:2,desc:'Code trav. L3121 — Réf. 35h · H.sup: +25% (≤43h) · +50% (>43h)'}
    },
    ...
    async _load(){
      try{
        ...
        if(snap.exists()){const d=snap.data();this._employees=d.employees||[];this._hrProfiles=d.hrProfiles||JSON.parse(JSON.stringify(this.HR_LEGAL));}
        else{this._hrProfiles=JSON.parse(JSON.stringify(this.HR_LEGAL));}
      }catch(e){
    ...
    validateSchedule(emp){
      ...
      dates.forEach(date=>{
        ...
        if(profile?.nightBan&&shift.start&&shift.end){if(shift.start<'06:00'||shift.start>='22:00'||shift.end>'22:00')warnings.push(`Nuit interdit ${date.toLocaleDateString('fr',{weekday:'short'})}`);}
    ...
    // ══════════════════════════════════════════════
    // ══ RENDER GRILLE (Connecteam-style) ══════════
    // ══════════════════════════════════════════════
    render:async function(){
      ...
      if(labelEl){
        ...
        labelEl.innerHTML=`${dates[0].toLocaleDateString('fr',opts)} – ${dates[6].toLocaleDateString('fr',opts)}${isCurrentWeek?' <span style="font-size:10px;background:#0891b2;color:#fff;border-radius:6px;padding:2px 6px;margin-left:6px;font-weight:900;">CETTE SEMAINE</span>':`<span style="font-size:10px;color:#94a3b8;margin-left:6px;font-weight:700;">${this._weekOffset>0?'+'+this._weekOffset:''}${this._weekOffset<0?this._weekOffset:''} sem.</span>`}`;
      }
      ...
      if(!this._employees.length){
        wrap.innerHTML=`<div style="text-align:center;padding:40px 20px;">
          ...
          <div style="font-weight:900;font-size:16px;color:#334155;margin-bottom:8px;">Aucun employé</div>
          <div style="font-size:13px;color:#94a3b8;margin-bottom:20px;">Ajoutez des employés pour commencer à planifier</div>
          <button onclick="app.planning.addEmployee()" style="background:#0891b2;color:#fff;border:none;border-radius:12px;padding:12px 24px;font-weight:900;font-size:14px;cursor:pointer;"><i class="fas fa-user-plus"></i> Ajouter un employé</button>
          <button onclick="app.planning.importFromUsers()" style="background:#059669;color:#fff;border:none;border-radius:12px;padding:12px 24px;font-weight:900;font-size:14px;cursor:pointer;margin-left:8px;"><i class="fas fa-users-gear"></i> Importer</button>
        </div>`;
      ...
      // ── Day headers row
      const headerHtml=`<div class="ct-header-row">
        <div class="ct-emp-col ct-emp-header">
          <span style="font-size:11px;font-weight:900;color:#94a3b8;">${this._employees.length} EMPLOYÉ${this._employees.length>1?'S':''}</span>
        </div>
        ${dates.map((d,i)=>{
          ...
          return`<div class="ct-day-col ${isT?'ct-today':''}">
            <div class="ct-day-name">${DAY_SHORT[i]}</div>
            <div class="ct-day-num">${d.getDate()}</div>
            ${cnt>0?`<div class="ct-day-stats">${cnt}p · ${tot.toFixed(0)}h</div>`:`<div class="ct-day-stats" style="opacity:.4;">—</div>`}
          </div>`;
        }).join('')}
        <div class="ct-total-header">Total<br>sem.</div>
      </div>`;
      ...
      const rowsHtml=this._employees.map(emp=>{
        ...
        const profile=emp.hrProfile?this._hrProfiles[emp.hrProfile]:null;
        const maxH=profile?profile.maxHoursWeek:48;
        const pct=Math.min(100,Math.round((totalHours/Math.max(maxH,1))*100));
        const hColor=totalHours>maxH?'#dc2626':totalHours>35?'#059669':'#0891b2';
        const isSelected=String(this._selectedEmpId)===String(emp.id);
        ...
        const cells=dates.map(date=>{
          ...
          if(isVac&&(!shift||shift.type==='rest')){
            return`<div class="ct-cell ct-leave ${isT?'ct-today':''}" data-emp="${emp.id}" data-key="${key}">
              <span class="ct-leave-icon">🌴</span>
              <span class="ct-leave-lbl">CONGÉ</span>
            </div>`;
          }
          if(!shift||shift.type==='rest'){
            return`<div class="ct-cell ct-rest ${isT?'ct-today':''}" data-emp="${emp.id}" data-key="${key}">
              <span class="ct-rest-dash">—</span>
            </div>`;
          }
          if(shift.type==='leave'){
            const icons={CP:'🌴',RTT:'📅',MALADIE:'🏥',SANS_SOLDE:'💸',FORMATION:'📚'};
            return`<div class="ct-cell ct-leave ${isT?'ct-today':''}" data-emp="${emp.id}" data-key="${key}">
              <span class="ct-leave-icon">${icons[shift.leaveType]||'🌴'}</span>
              <span class="ct-leave-lbl">${shift.leaveType||'CONGÉ'}</span>
            </div>`;
          }
          const h=this.getShiftHours(shift);
          ...
          // Visual time bar
          const startPct=(() => { const[sh2,sm2]=shift.start.split(':').map(Number); return Math.round(((sh2*60+sm2-360)/960)*100); })();
          const durPct=Math.round((h/16)*100);
          return`<div class="ct-cell ct-work ${isT?'ct-today':''}" style="--sc:${c};" data-emp="${emp.id}" data-key="${key}" data-movable="1">
            <div class="ct-shift-bar" style="left:${Math.max(0,startPct)}%;width:${Math.min(100,durPct)}%;background:${c};"></div>
            <div class="ct-shift-post" style="color:${c};">${shift.post||'TRAVAIL'}</div>
            <div class="ct-shift-times">${shift.start}–${shift.end}</div>
            <div class="ct-shift-h">${h.toFixed(1)}h${brk?` ☕${brk}'`:''}</div>
          </div>`;
        }).join('');

        return`<div class="ct-row ${warnings.length?'ct-has-warn':''} ${isSelected?'ct-row-selected':''}">
          <div class="ct-emp-col" onclick="app.planning._selectEmp('${emp.id}')">
            <div class="ct-avatar" style="background:${col};">${initials}</div>
            <div class="ct-emp-info">
              <div class="ct-emp-name">${emp.name}</div>
              <div class="ct-emp-contract">${emp.contract||'Pas de contrat'}${profile?' · '+profile.icon:''}
              </div>
              <div class="ct-progress-bar">
                <div class="ct-progress-fill" style="width:${pct}%;background:${hColor};"></div>
              </div>
              <div class="ct-hours-label" style="color:${hColor};">${totalHours.toFixed(1)}h / ${maxH}h</div>
            </div>
            <button class="ct-emp-settings-btn" onclick="event.stopPropagation();app.planning.openEmpSettings('${emp.id}')" title="Paramètres">
              <i class="fas fa-ellipsis-v"></i>
            </button>
          </div>
          ${cells}
          <div class="ct-total-col">
            <div class="ct-total-h" style="color:${hColor};">${totalHours.toFixed(0)}h</div>
            <div class="ct-rest-badge ${restDays>=2?'ok':'warn'}">${restDays}R</div>
          </div>
        </div>
        ${warnings.length?`<div class="ct-warn-row"><i class="fas fa-triangle-exclamation"></i> ${warnings.join(' · ')}</div>`:''}`;
      }).join('');
      ...
      if(se&&sc){
        ...
        const warnCount=this._employees.filter(e=>this.validateSchedule(e).warnings.length>0).length;
        sc.innerHTML=`<div class="ct-summary-bar">
          <div class="ct-sum-item"><div class="ct-sum-val" style="color:#0891b2;">${this._employees.length}</div><div class="ct-sum-lbl">Employés</div></div>
          <div class="ct-sum-item"><div class="ct-sum-val" style="color:#059669;">${tw.toFixed(0)}h</div><div class="ct-sum-lbl">Total semaine</div></div>
          <div class="ct-sum-item"><div class="ct-sum-val" style="color:#7c3aed;">${costTotal>0?costTotal.toFixed(0)+'€':'—'}</div><div class="ct-sum-lbl">Coût brut</div></div>
          <div class="ct-sum-item"><div class="ct-sum-val" style="color:${warnCount>0?'#dc2626':'#059669'};">${warnCount>0?'⚠ '+warnCount:'✓ OK'}</div><div class="ct-sum-lbl">Alertes RH</div></div>
        </div>`;
      }
    },

    _selectEmp(empId){
      this._selectedEmpId=this._selectedEmpId===empId?null:empId;
      this.render();
    },
    ...
    _bindCells(wrap){
      ...
      if(!ctxMenu){
        ...
        ctxMenu.style.cssText='position:fixed;z-index:99999;background:#fff;border-radius:14px;box-shadow:0 8px 30px rgba(0,0,0,.2);padding:8px;min-width:190px;display:none;';
        document.body.appendChild(ctxMenu);
        document.addEventListener('click',()=>{ctxMenu.style.display='none';},{passive:true});
      }

      wrap.querySelectorAll('.ct-cell').forEach(el=>{
        // Tap
        ...
        el.addEventListener('contextmenu',(e)=>{
          ...
          const empId=el.dataset.emp;const key=el.dataset.key;
          if(!empId||!key)return;
          const emp=this._employees.find(e2=>String(e2.id)===String(empId));
          const shift=emp?.schedule?.[key];
          const isWork=shift?.type==='work';
          const dateLabel=new Date(key+'T12:00').toLocaleDateString('fr',{weekday:'short',day:'numeric',month:'short'});
          ctxMenu.innerHTML=`
            <div style="font-size:11px;font-weight:900;color:#94a3b8;padding:4px 10px 6px;border-bottom:1px solid #f1f5f9;margin-bottom:4px;">${emp?.name||''} · ${dateLabel}</div>
            <div onclick="app.planning.editShift('${empId}','${key}');document.getElementById('plan-ctx-menu').style.display='none'" style="display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:10px;cursor:pointer;font-weight:800;font-size:13px;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
              <i class="fas fa-pen" style="color:#0891b2;width:16px;"></i> Modifier
            </div>
            ${isWork?`<div onclick="app.planning._duplicateShiftDirect('${empId}','${key}');document.getElementById('plan-ctx-menu').style.display='none'" style="display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:10px;cursor:pointer;font-weight:800;font-size:13px;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
              <i class="fas fa-clone" style="color:#059669;width:16px;"></i> Dupliquer sur jour suivant
            </div>`:''}
            <div onclick="app.planning._copyShiftToWeek('${empId}','${key}');document.getElementById('plan-ctx-menu').style.display='none'" style="display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:10px;cursor:pointer;font-weight:800;font-size:13px;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">
              <i class="fas fa-copy" style="color:#7c3aed;width:16px;"></i> Appliquer à toute la semaine
            </div>
            <div style="height:1px;background:#f1f5f9;margin:4px 0;"></div>
            <div onclick="app.planning._deleteShiftDirect('${empId}','${key}');document.getElementById('plan-ctx-menu').style.display='none'" style="display:flex;align-items:center;gap:10px;padding:9px 10px;border-radius:10px;cursor:pointer;font-weight:800;font-size:13px;" onmouseover="this.style.background='#fff1f2'" onmouseout="this.style.background='transparent'">
        ...
        el.addEventListener('touchstart',(e)=>{
          longPress=setTimeout(()=>{
            ...
            ghost.style.cssText=`position:fixed;opacity:.85;pointer-events:none;z-index:99999;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.35);width:${el.offsetWidth}px;transform:scale(1.08);transition:transform .1s;`;
        ...
        el.addEventListener('touchmove',(e)=>{
          ...
          if(overEl)overEl.classList.remove('ct-drag-over');
          const hit=document.elementFromPoint(t.clientX,t.clientY);
          overEl=hit?.closest?.('.ct-cell');
          if(overEl&&overEl!==dragEl)overEl.classList.add('ct-drag-over');
        },{passive:false});
        el.addEventListener('touchend',()=>{
          clearTimeout(longPress);longPress=null;
          if(ghost){ghost.remove();ghost=null;}
          if(dragEl){dragEl.style.opacity='';dragEl._dragged=!!overEl&&overEl!==dragEl;}
          if(overEl)overEl.classList.remove('ct-drag-over');
    ...
    _copyShiftToWeek:async function(empId,dateKey){
      const emp=this._employees.find(e=>String(e.id)===String(empId));if(!emp)return;
      const shift=emp.schedule?.[dateKey];if(!shift||shift.type!=='work')return toast('Aucun shift à copier.','warn');
      const ok=await dlgConfirm('Appliquer à la semaine',`Copier ${shift.start}–${shift.end} (${shift.post||''}) sur tous les jours ouvrés de la semaine ?`);
      if(!ok)return;
      ...
      dates.forEach(d=>{
        ...
        if(k===dateKey)return;
        if(!this._isVacation(emp,k))emp.schedule[k]={...shift};
      });
      await this._save();this.render();
      toast('Shift copié sur la semaine ✓','ok',2500);
    },
    ...
    editShift(empId,dateKey){
      ...
      if(isVac&&(!shift||shift.type==='rest')){toast('🌴 Date bloquée par congés. Modifiez les paramètres employé.','warn',3500);return;}
      ...
      document.getElementById('shift-modal-emp').innerHTML=`<span style="font-weight:950;">${emp.name}</span> <span style="font-weight:400;color:#64748b;font-size:13px;">— ${dateLabel}</span>`;
    ...
    saveShift:async function(){
      ...
      if(isLeave){
        ...
        if((emp.workFrom&&start<emp.workFrom)||(emp.workTo&&end>emp.workTo)){
          const ok=await dlgConfirm('Hors plage autorisée','Ce shift dépasse la plage horaire configurée. Continuer ?',false);if(!ok)return;
        }
      ...
      toast('Shift enregistré ✓','ok',1500);
    },
    ...
    renderRecap(period='week'){
      ...
      else if(period==='month'){const y=today.getFullYear(),m=today.getMonth();for(let d=new Date(y,m,1);d<=new Date(y,m+1,0);d.setDate(d.getDate()+1))dates.push(new Date(d));}
      ...
      const rows=this._employees.map(emp=>{
        ...
        return`<div style="background:#fff;border-radius:14px;padding:14px;margin-bottom:10px;box-shadow:0 2px 8px rgba(0,0,0,.05);">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            ...
            <div style="flex:1;"><div style="font-weight:950;font-size:14px;">${emp.name}</div><div style="font-size:11px;color:#64748b;">${emp.contract||''}${emp.rate?' · '+emp.rate+'€/h':''}</div></div>
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:8px;">
            <div style="background:#eff6ff;border-radius:8px;padding:8px;text-align:center;"><div style="font-size:18px;font-weight:950;color:#0891b2;">${plannedH.toFixed(1)}</div><div style="font-size:10px;color:#64748b;">h planif.</div></div>
            <div style="background:#f0fdf4;border-radius:8px;padding:8px;text-align:center;"><div style="font-size:18px;font-weight:950;color:#059669;">${workedH>0?workedH.toFixed(1):'—'}</div><div style="font-size:10px;color:#64748b;">h réelles</div></div>
            <div style="background:${Math.abs(diff)>0.25?'#fffbeb':'#f8fafc'};border-radius:8px;padding:8px;text-align:center;"><div style="font-size:18px;font-weight:950;color:${diffColor};">${workedH>0?(diff>=0?'+':'')+diff.toFixed(1):'—'}</div><div style="font-size:10px;color:#64748b;">écart</div></div>
            <div style="background:${leaveDays?'#fdf2f8':'#f8fafc'};border-radius:8px;padding:8px;text-align:center;"><div style="font-size:18px;font-weight:950;color:#db2777;">${leaveDays||'—'}</div><div style="font-size:10px;color:#64748b;">j congés</div></div>
          </div>
      ...
      wrap.innerHTML=Object.entries(this._hrProfiles).map(([key,p])=>{
        ...
        return`<div style="background:#fff;border-radius:14px;padding:14px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,.06);border-left:4px solid ${p.color||'#7c3aed'};">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
            <div><label style="font-size:11px;color:#64748b;display:block;margin-bottom:2px;">Max h/jour</label><input type="number" value="${p.maxHoursDay}" style="margin:0;padding:8px;font-size:14px;" oninput="app.planning._updateProfile('${key}','maxHoursDay',parseFloat(this.value))"></div>
            <div><label style="font-size:11px;color:#64748b;display:block;margin-bottom:2px;">Max h/semaine</label><input type="number" value="${p.maxHoursWeek}" style="margin:0;padding:8px;font-size:14px;" oninput="app.planning._updateProfile('${key}','maxHoursWeek',parseFloat(this.value))"></div>
            <div><label style="font-size:11px;color:#64748b;display:block;margin-bottom:2px;">Repos min entre shifts (h)</label><input type="number" value="${p.minRestHours}" style="margin:0;padding:8px;font-size:14px;" oninput="app.planning._updateProfile('${key}','minRestHours',parseFloat(this.value))"></div>
            <div><label style="font-size:11px;color:#64748b;display:block;margin-bottom:2px;">Pause après (h)</label><input type="number" value="${p.breakAfterH}" style="margin:0;padding:8px;font-size:14px;" oninput="app.planning._updateProfile('${key}','breakAfterH',parseFloat(this.value))"></div>
          </div>
    ...
    assignProfileToEmp:async function(profileKey){
      ...
      if(!m){m=document.createElement('div');m.id='modal-assign-profile';m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:90001;display:flex;align-items:flex-end;justify-content:center;padding:16px;';document.body.appendChild(m);}
      const p=this._hrProfiles[profileKey];
      const empHtml=this._employees.map(e=>{
        const hasP=e.hrProfile===profileKey;
        return`<div style="display:flex;align-items:center;gap:12px;padding:12px;border-radius:10px;margin-bottom:6px;background:${hasP?'#ede9fe':'#f8fafc'};border:2px solid ${hasP?'#a78bfa':'#e2e8f0'};cursor:pointer;" onclick="app.planning._setProfileDirectly('${e.id}','${profileKey}')">
          ...
          <div style="flex:1;"><div style="font-weight:950;font-size:13px;">${e.name}</div><div style="font-size:11px;color:#64748b;">${e.contract||'Pas de contrat défini'}</div></div>
    ...
    importFromUsers:async function(){
      ...
      if(!m){m=document.createElement('div');m.id='modal-import-users';m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:90000;display:flex;align-items:flex-end;justify-content:center;padding:16px;';document.body.appendChild(m);}
      ...
      const userHtml=users.map(u=>{
        ...
        return`<div id="imp-user-${u.uid}" style="display:flex;align-items:center;gap:12px;padding:12px;border-radius:10px;margin-bottom:6px;background:${isIn?'#f0fdf4':'#f8fafc'};border:2px solid ${isIn?'#86efac':'#e2e8f0'};cursor:${isIn?'default':'pointer'};" ${!isIn?`onclick="this.classList.toggle('selected-imp');this.style.background=this.classList.contains('selected-imp')?'#dbeafe':'#f8fafc';this.style.borderColor=this.classList.contains('selected-imp')?'#93c5fd':'#e2e8f0';"`:''}">
          ...
          <div style="flex:1;"><div style="font-weight:950;font-size:13px;">${name}</div><div style="font-size:11px;color:#64748b;">${u.role||'staff'} • ${u.email||''}</div></div>
    ...
    _doImportSelected:async function(){
      const m=document.getElementById('modal-import-users');if(!m)return;
    ...
    copyWeek:async function(){
      ...
      if(dir===1){
        ...
        this._employees.forEach(emp=>{if(!emp.schedule)emp.schedule={};dates.forEach(date=>{const key=date.toISOString().slice(0,10);const nk=new Date(date);nk.setDate(date.getDate()+7);const nextKey=nk.toISOString().slice(0,10);if(emp.schedule[key])emp.schedule[nextKey]={...emp.schedule[key]};else delete emp.schedule[nextKey];});});
        ...
        this._employees.forEach(emp=>{if(!emp.schedule)emp.schedule={};dates.forEach(date=>{const pk=new Date(date);pk.setDate(date.getDate()-7);const key=date.toISOString().slice(0,10);const prevKey=pk.toISOString().slice(0,10);if(emp.schedule[prevKey])emp.schedule[key]={...emp.schedule[prevKey]};else delete emp.schedule[key];});});
    ...
    exportCSV(){
      ...
      this._employees.forEach(emp=>{
        ...
        const row=[emp.name,emp.contract||'',emp.hrProfile||'',emp.rate||'',...dates.map(d=>{const key=d.toISOString().slice(0,10);const s=emp.schedule?.[key]||{type:'rest'};if(s.type==='leave')return`CONGÉ-${s.leaveType||''}`;return s.type==='rest'?'Repos':`${s.start}-${s.end} ${s.post||''}${s.breakMin?` pause:${s.breakMin}min`:''}`;}),totalHours.toFixed(1)+'h',emp.rate?(totalHours*emp.rate).toFixed(2)+'€':''];
        rows.push(row);
      });
      const csv=rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(';')).join('\n');
      const blob=new Blob(['\uFEFF'+csv],{type:'text/csv;charset=utf-8;'});
    ...
    printWeek(){
      ...
      const headers=DAY.map((d,i)=>{const isT=dates[i].toISOString().slice(0,10)===today;return`<th style="${isT?'background:#0891b2;color:#fff;':''}">${d} ${dates[i].getDate()}</th>`;}).join('');
      const rows=this._employees.map(emp=>{
        const{totalHours,restDays,warnings}=this.validateSchedule(emp);
        const profile=emp.hrProfile?this._hrProfiles[emp.hrProfile]:null;
      ...
      const dayTot=dates.map(d=>{const key=d.toISOString().slice(0,10);const tot=this._employees.reduce((s,e)=>{const sh=e.schedule?.[key];return s+(sh?.type==='work'?this.getShiftHours(sh):0);},0);const cnt=this._employees.filter(e=>e.schedule?.[key]?.type==='work').length;return`<td style="font-size:9px;">${cnt}p · ${tot.toFixed(0)}h</td>`;}).join('');
      ...
      <table><thead><tr><th>Employé</th>${headers}<th>Total</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><td><strong>Par jour</strong></td>${dayTot}<td></td></tr></tfoot></table>`;
  ...
  pointeuse:{
    ...
    async _renderLogs(){
      ...
      try{
        ...
        const labels={in:'Arrivée',out:'Départ',pause_start:'Début pause',pause_end:'Fin pause'};
        const isAdmin=window.app.canDo('pointeuseAdmin');
        wrap.innerHTML=Object.values(byUser).map(u=>{
          const uLogs=u.logs.sort((a,b)=>a.ts-b.ts);
          // Compute current state for this user
          const uIns=uLogs.filter(l=>l.type==='in');const uOuts=uLogs.filter(l=>l.type==='out');
          const uPS=uLogs.filter(l=>l.type==='pause_start');const uPE=uLogs.filter(l=>l.type==='pause_end');
          const isActive=uIns.length>0&&(uOuts.length===0||uIns[uIns.length-1].ts>uOuts[uOuts.length-1].ts);
          const isPaused=isActive&&uPS.length>0&&(uPE.length===0||uPS[uPS.length-1].ts>uPE[uPE.length-1].ts);
          const uid=uIns.length?uIns[0].userId:uLogs[0]?.userId||'';
          return`<div style="margin-bottom:16px;background:rgba(255,255,255,.04);border-radius:12px;padding:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
              <div style="font-weight:950;color:rgba(255,255,255,.8);font-size:13px;">${u.name}
                ${isActive?`<span style="font-size:10px;background:rgba(16,185,129,.2);color:#6ee7b7;border-radius:6px;padding:2px 6px;margin-left:6px;">${isPaused?'⏸ PAUSE':'🟢 EN POSTE'}</span>`:'<span style="font-size:10px;background:rgba(255,255,255,.08);color:rgba(255,255,255,.4);border-radius:6px;padding:2px 6px;margin-left:6px;">⚫ SORTI</span>'}
              </div>
              ${isAdmin&&isActive?`<div style="display:flex;gap:5px;">
                ${isPaused?`<button onclick="app.pointeuse.adminEndPause('${uid}','${u.name}')" style="background:rgba(251,191,36,.2);color:#fbbf24;border:none;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:900;cursor:pointer;"><i class="fas fa-play"></i> Fin pause</button>`:''}
                <button onclick="app.pointeuse.adminDepoint('${uid}','${u.name}')" style="background:rgba(239,68,68,.2);color:#f87171;border:none;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:900;cursor:pointer;"><i class="fas fa-stop"></i> Dépointage</button>
              </div>`:''}
            </div>
            ${uLogs.map(l=>`<div class="punch-log">
              <div><div class="punch-time">${this._fmtTime(l.ts)}</div>
              <div class="punch-label ${l.type==='in'?'in':l.type==='out'?'out':'pause'}">${labels[l.type]||l.type}${l.adminForced?' (admin)':''}</div></div>
            </div>`).join('')}
          </div>`;
        }).join('');
    ...
    // ── DÉPOINTAGE ADMIN ──
    async adminDepoint(userId, userName){
      if(!window.app.canDo('pointeuseAdmin')){toast('Permission admin requise.','err');return;}
      const ok=await dlgConfirm('Dépointage','Forcer le départ de '+userName+' ?',true);
      if(!ok)return;
      const today=this._todayStr();
      const{addDoc,serverTimestamp}=window._fbChat;
      try{
        await addDoc(this.colRef(),{
          type:'out',userId,userName,
          ts:Date.now(),date:today,serverTs:serverTimestamp(),
          adminForced:true,adminBy:window.app.me?.name||'ADMIN'
        });
        toast('Dépointage forcé pour '+userName,'ok',3000);
        await this._renderLogs();
      }catch(e){toast('Erreur dépointage.','err');}
    },

    // ── FIN PAUSE ADMIN ──
    async adminEndPause(userId, userName){
      if(!window.app.canDo('pointeuseAdmin')){toast('Permission admin requise.','err');return;}
      const ok=await dlgConfirm('Fin de pause','Terminer la pause de '+userName+' ?',false);
      if(!ok)return;
      const{addDoc,serverTimestamp}=window._fbChat;
      try{
        await addDoc(this.colRef(),{
          type:'pause_end',userId,userName,
          ts:Date.now(),date:this._todayStr(),serverTs:serverTimestamp(),
          adminForced:true
        });
        toast('Pause terminée pour '+userName,'ok');
        await this._renderLogs();
      }catch(e){toast('Erreur.','err');}
    },
...
onAuthStateChanged(auth,async user=>{
  if(!user){
    ...
    window.app.auth.initLoginScreen();
